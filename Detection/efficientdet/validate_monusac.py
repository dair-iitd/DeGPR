...
def validate(args):
    ...
    with torch.no_grad():
        for i, (input, target, name) in enumerate(loader):
            ...
            for j in range(len(name)):
                ...
                num_0 = torch.numel(dummy[dummy == 1])
                num_1 = torch.numel(dummy[dummy == 2])
                num_2 = torch.numel(dummy[dummy == 3])
                num_3 = torch.numel(dummy[dummy == 4])
                ...
                pred_0 = np.count_nonzero(dummy_pred[:,-1][np.where(dummy_pred[:,-2] > conf0)] == 1)
                pred_1 = np.count_nonzero(dummy_pred[:,-1][np.where(dummy_pred[:,-2] > conf1)] == 2)
                pred_2 = np.count_nonzero(dummy_pred[:,-1][np.where(dummy_pred[:,-2] > conf2)] == 3)
                pred_3 = np.count_nonzero(dummy_pred[:,-1][np.where(dummy_pred[:,-2] > conf3)] == 4)

                x = name[j].split('_')
                if len(x) > 0:
                    img_part = ''.join(name[j].split('_')[:-1])
                else:
                    img_part = name[j]
                if img_part not in count_metric:
                    count_metric[img_part] = [0,0,0,0,0,0,0,0]

                count_metric[img_part][0] += num_0
                count_metric[img_part][1] += pred_0
                count_metric[img_part][2] += num_1
                count_metric[img_part][3] += pred_1
                count_metric[img_part][4] += num_2
                count_metric[img_part][5] += pred_2
                count_metric[img_part][6] += num_3
                count_metric[img_part][7] += pred_3
    ...
    for img_name in count_metric:
        ...
        num_images+=1
        print(img_name, l[0], l[1], l[2], l[3], l[4], l[5], l[6], l[7])
        mae_1 += (abs(l[0] - l[1]))
        mae_2 += (abs(l[2] - l[3]))
        mae_3 += (abs(l[4] - l[5]))
        mae_4 += (abs(l[6] - l[7]))
    ...
    print(mae_1, mae_2, mae_3, mae_4, num_images)
    return mean_ap
