...
class CocoEvaluator(Evaluator):
    ...
    def evaluate(self, output_result_file=''):
        if not self.distributed or dist.get_rank() == 0:
            ...
            # coco_eval.params.iouThrs = np.linspace(.3, 0.75, int(np.round((0.75 - .3) / .05)) + 1, endpoint=True)
            coco_eval.evaluate()
            coco_eval.accumulate()
            coco_eval.summarize()
            # print(coco_eval.params.iouThrs)
            metric = coco_eval.stats[0]  # mAP 0.5-0.95
            # metric = coco_eval.stats[1]
            if self.distributed:
...
class TfmEvaluator(Evaluator):
    ...
    def __init__(
        ...
        super().__init__(distributed=distributed, pred_yxyx=pred_yxyx)
        self._evaluator = evaluator_cls(categories=dataset.parser.cat_dicts,matching_iou_threshold=0.3)
