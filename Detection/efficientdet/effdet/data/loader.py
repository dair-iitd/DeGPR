...
class DetectionFastCollate:
    ...
    def __call__(self, batch):
        ...
        for i in range(batch_size):
            ...
            # this line is added to facilitate postreg
            if i == 0:
                target['cls'] = torch.zeros((batch_size,2000) , dtype=torch.int64)
                target['bbox'] = torch.zeros((batch_size,2000,4) , dtype=torch.float32)
                target['num_objects'] = torch.zeros(batch_size, dtype=torch.int64)

            if 'cls' in labeler_inputs.keys():
                num_objects = labeler_inputs['cls'].shape[0]
                target['num_objects'][i] = num_objects
                target['cls'][i][0:num_objects] = labeler_inputs['cls']
                target['bbox'][i][0:num_objects,:] = labeler_inputs['bbox']
        ...
        return img_tensor, target, img_names
...
class PrefetchLoader:
    ...
    def __iter__(self):
        ...
        for next_input, next_target, next_name in self.loader:
